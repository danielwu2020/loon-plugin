...
function boolLine(name, boolValue, reverse) {
  const risky = reverse ? !boolValue : !!boolValue;
  return (risky ? "🔴 " : "🟢 ") + name + "：" + (boolValue ? "是" : "否");
}

function textLine(name, value, level) {
  return icon(level) + " " + name + "：" + value;
}
...
function analyzeRisk(ipApi, cz88, ipqs, abuse) {
  ...
  if (ipqs) {
    ...
    suspiciousProxy = ipqs.proxy === true || ipqs.vpn === true || ipqs.tor === true || ipqs.active_vpn === true || ipqs.active_tor === true;
    ...
    if (ipqs.fraud_score >= 75) level = "较差";
    else if (ipqs.fraud_score >= 45) level = "一般";
    else if (ipqs.fraud_score >= 20) level = "良好";
    else level = "优秀";
